...
const handleCheckoutSessionCompleted = async (session) => {
  try {
    ...
    if (order) {
      // Only update stock if order is still pending (not already processed)
...
const handlePaymentIntentSucceeded = async (paymentIntent) => {
  try {
    ...
    if (order) {
      // Only update stock if order is still pending (not already processed)
      // This prevents duplicate stock updates when payment succeeds immediately
      if (order.orderStatus === 'Pending') {
        ...
        // Update stock and delete cart after successful payment
        const userCart = await Cart.findOne({ orderBy: order.orderBy });
        if (userCart) {
          await updateProductStock(userCart.products);
          await Cart.deleteOne({ orderBy: order.orderBy });
